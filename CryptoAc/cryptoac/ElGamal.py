...
from algorithms.euclids import Euclids

class ElGamalEncryption:
    def __init__(self):
        self._rand = RandomGenerator()
    
    def genKey(self, nbit):
        p = Primes().get_prime(nbit)
        g = self._rand.ranrange(10, p-1)
        a = self._rand.ranrange(10, p-1)
        A = BytesUtils.myPow(g, a, p)
        public = {'p': p, 'g': g, 'A': A}
        private = {'p': p, 'a': a, 'g': g}
        return {'public': public, 'private': private}
    
    def __encrypt__(self, g, p, A, m):
        k = self._rand.ranrange(2, p-1)
        c1 = BytesUtils.myPow(g, k, p)
        c2 = (m * BytesUtils.myPow(A, k, p)) % p
        return c1, c2
    
    def __decrypt__(self, a, p, c1, c2):
        x1 = BytesUtils.myPow(c1, a, p)
